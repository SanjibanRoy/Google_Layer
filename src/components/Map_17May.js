...
function Map({ updateBox }) {
  ...
  useEffect(() => {
    ...
    if (!mounted.current) {
      setmainMap(L.map("map", mapParams));
      ...
      if (analyticsLayer != null) {
        mainMap.removeLayer(analyticsLayer);
        mainMap.addLayer(analyticsLayer);
      } else {
        setanalyticsLayer(
          L.tileLayer(
            "https://vedas.sac.gov.in/InteractiveGeoService/tms_comp_diff/NDVI_MODIS/{z}/{x}/{-y}?tm_arr1=1616976000&tm_arr2=1585440000&opr=max&min_val=0&max_val=250&color_map_name=NDVI_DIFF"
          )
        );
        // mainMap.addLayer(analyticsLayer);
      }
